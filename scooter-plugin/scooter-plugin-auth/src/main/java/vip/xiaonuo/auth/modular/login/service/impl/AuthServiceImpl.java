...
public class AuthServiceImpl implements AuthService {
    ...
    public static final String SNOWY_SYS_DEFAULT_CAPTCHA_OPEN_KEY = "SNOWY_SYS_DEFAULT_CAPTCHA_OPEN";
     ...
     **/
    public void validPhoneValidCodeParam(String phoneOrEmail, String validCode, String validCodeReqNo, String type) {
        // 验证码正确则校验手机号格式
        if (ObjectUtil.isEmpty(phoneOrEmail)) {
            // 执行校验验证码
    ...
    public String doLogin(AuthAccountPasswordLoginParam authAccountPasswordLoginParam, String type) {
        ...
        if (SaClientTypeEnum.B.getValue().equals(type)) {
            ...
            if (ObjectUtil.isNotEmpty(authAccountPasswordLoginParam.getAccount())) {
                // 使用账号登录
                // 获取账号
                String account = authAccountPasswordLoginParam.getAccount();
                saBaseClientLoginUser = clientLoginUserApi.getClientUserByAccount(account);
            } else {
                // 使用邮箱登录
                String email = authAccountPasswordLoginParam.getEmail();
                saBaseClientLoginUser = clientLoginUserApi.getClientUserByEmail(email);
            }
            if (ObjectUtil.isEmpty(saBaseClientLoginUser)) {
                throw new CommonException(AuthExceptionEnum.EMAIL_ERROR.getValue());
            }
